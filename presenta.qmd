...
Hasta ahora nos hemos enfocado en los siguientes modelos:
...
En el modelo de dos parámetros, el valor de $Y_i$ depende tanto de una constante/intercepto y del valor del predictor $X_i$.

Un predictor $X_i$ puede ser categórico (en R, estas variables son de tipo `factor`) o puede ser continuo (en R, estas variables son de tipo `integer` o `double`).
...
1.  siempre se busca minimizar la suma de errores al cuadrados y
...
abline(reg = modeloA, col = "blue", lwd = 2)
segments(x0 = 0, y0 = -3, x1 = 0, y1 = -0.3510612, col="gray", lwd=2)
segments(x0 = 0, y0 = -0.3510612, x1 = -3, y1 = -0.3510612, col="gray", lwd=2)
```
...
El valor debajo de `Pr(>F)` (2.245e-05) se puede expresar en notación decimal con la función `format()`. También puede ejecutar `options(scipen = 999)` si desea ver todos los resultados en notación decimal.
...
Nótese que el valor 0 no está incluido en el intervalo de confianza de $b_1$, el cual corresponde al coeficiente asociado al predictor `nw_accuracy_pretest`. Esto es congruente con el rechazo de la hipótesis nula de que de que $\beta_1 = 0$.
...
b1 <- coef(modelo)[["x"]]
lb <- confint(modelo)["x", "2.5 %"]
ub <- confint(modelo)["x", "97.5 %"]

c(lb, b1, ub)
between(0.3650, lb, ub)
```
...
for (i in 1:1000) {
  ...
  incluye[i] <- between(0.3650, lb, ub)
}
